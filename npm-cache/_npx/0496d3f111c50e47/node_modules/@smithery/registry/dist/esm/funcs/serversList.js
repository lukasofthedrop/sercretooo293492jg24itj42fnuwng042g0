 ...
 */
export function serversList(client, request, options) {
    return new APIPromise($do(client, request, options));
}
async function $do(client, request, options) {
    const parsed = safeParse(request, (value) => operations.ListServersRequest$outboundSchema.parse(value), "Input validation failed");
    if (!parsed.ok) {
    ...
    const [result, raw] = await M.match(M.json(200, operations.ListServersResponse$inboundSchema, {
        key: "Result",
    }), M.jsonErr(401, errors.UnauthorizedError$inboundSchema), M.jsonErr(500, errors.ServerError$inboundSchema), M.fail("4XX"), M.fail("5XX"))(response, req, { extraFields: responseFields });
    if (!result.ok) {
